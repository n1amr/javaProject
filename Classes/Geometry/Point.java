public class Point implements Comparable<Point> {
	...
	public double dot(Point b) {
		return (this.conjugate().multiply(b).x);
	}

	public double cross(Point b) {
		return (this.conjugate().multiply(b).y);
	}
	...
	public Point reflect(Point mirror) {
		return (this.divide(mirror)).conjugate().multiply(mirror);
	}
	...
	public boolean isOnRay(Point a, Point b) {
		return this.isOnLine(a, b) && vec(a, b).dot(vec(a, this)) > -EPS;
	}

	public boolean isOnSegment(Point a, Point b) {
		if (a.equals(b))
			return a.equals(this);
		return this.isOnRay(a, b) && this.isOnRay(b, a);
	}
	...
	public Point intersection(Point a, Point b, Point p, Point q) {
		...
		if (Math.abs(d1 - d2) < EPS)
			return null;
		return q.multiply(d1).subtract(p.multiply(d2))
				.multiply(1.0 / (d1 - d2));
	}
	...
	public static Point exp(Point a) {
		return new Point(Math.exp(a.x) * Math.cos(a.y), Math.exp(a.x)
				* Math.sin(a.y));
	}

	public static Point sin(Point a) {
		return new Point(Math.sin(a.x) * Math.cosh(a.y), Math.cos(a.x)
				* Math.sinh(a.y));
	}

	public static Point cos(Point a) {
		return new Point(Math.cos(a.x) * Math.cosh(a.y), -Math.sin(a.x)
				* Math.sinh(a.y));
	}
